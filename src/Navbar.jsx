...
const Navbar = ({ onSearch }) => {
  ...
  return (
    <div>
      ...
      {/* Mobile Sidebar */}
      <div
        className={`fixed top-0 left-0 w-64 h-full bg-boxbg text-mytext z-50 transform ${
      ...
      >
        ...
        <div className="p-4">
          <input
            type="text"
            placeholder="Search Product"
            className="pl-4 pr-8 py-2 rounded-full border bg-boxbg text-mytext border-gray-500 focus:outline-none w-full"
            value={searchTerm}
            onChange={handleSearchChange}
          />
        </div>
        <nav className="flex flex-col p-4 space-y-4">
          <Link
            to="/vendors"
            className="flex items-center space-x-2  hover:bg-slate-300 p-2 rounded"
          >
            <FaStore />
            <span>Vendors</span>
          </Link>
          ...
          <button className="flex items-center space-x-2  hover:bg-slate-300 p-2 rounded mt-auto">
            <FaSignInAlt />
